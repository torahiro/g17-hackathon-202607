...
# 定数
# ==============================
# app.py が置かれているディレクトリを基準にする
# → streamlit run をどのディレクトリから実行しても、常に app/ 内のファイルを正しく参照できる
APP_DIR = os.path.dirname(os.path.abspath(__file__))

CACHE_FILE = os.path.join(APP_DIR, "ocr_cache.txt")            # OCR結果のキャッシュ
SUMMARY_CACHE_FILE = os.path.join(APP_DIR, "summary_cache.txt")  # 要約結果のキャッシュ
NANDEMO_HTML_PATH = os.path.join(APP_DIR, "nandemo_v.html")      # HUD画面のHTML

# 同梱ルールブックPDFのパス（本番用に固定）
# ※ このファイル名・配置場所は実際に用意するPDFに合わせて変更してください
DEFAULT_PDF_PATH = os.path.join(APP_DIR, "クトゥルフ神話TRPGルール.pdf")
...
with col2:
    ...
    try:
        with open(NANDEMO_HTML_PATH, "r", encoding="utf-8") as f:
            html_content = f.read()
        ...
        st.error(f"`nandemo_v.html` が見つかりません（探した場所: {NANDEMO_HTML_PATH}）。")